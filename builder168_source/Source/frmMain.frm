...
Public Sub itmEditMapOptions_Click()
     ...
     Dim OldTexDir As String
     Dim OldFlatDir As String
     ...
     OldMixResources = mapconfig("mixmixtexturesflats")
     OldAddWad = addwadfile
     OldTexDir = addtexdir
     OldFlatDir = addflatdir
     ...
     If (ChangeMapOptions) And (mapfile <> "") Then
          ...
          'Reload textures and flat when game (IWAD) changed or additional file changed
          If (OldGame <> mapgame) Or (OldAddWad <> addwadfile) Or _
             (OldTexDir <> addtexdir) Or (OldFlatDir <> addflatdir) Or _
             (OldMixResources <> mapconfig("mixmixtexturesflats")) Then
